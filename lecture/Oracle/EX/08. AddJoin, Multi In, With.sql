...
--학생별 기말고사 성적의 평균의 55점이상인 학생번호, 학생이름, 기말고사 평균 조회(NATURAL JOIN)
SELECT SNO
    , ST.SNAME
    , AVG(SC.RESULT)
    FROM SCORE SC
...
--학점이 3학점인 과목의 교수번호, 교수이름, 과목번호, 과목이름, 학점 조회(JOIN ~ USING 사용)
SELECT PNO
    , P.PNAME
    , C.CNO
    , C.CNAME
...
--2. 다중 컬럼 IN절
--dno = 01이면서 job이 경영인 사원이나 dno = 30 이면서 job이 회계인 사원 조회
SELECT DNO
    , ENO
    ,ENAME
    , JOB
...
--다중 컬럼 IN절(CNO, PNO)을 이용해서 기말고사 성적이 평균이 48점 이상인 
...
--기말고사 성적중 과목명에 화학이 포함되는 성적 정보를 가상 테이블로 생성(SCORE의 CNO, SNO, RESULT // COURSE의 CNAME)
--학생별 화학이 포함된 과목의 기말고사 성적의 평균(학생번호, 학생이름, 평균성적)

WITH
    ST AS (SELECT * FROM STUDENT WHERE MAJOR = '화학')
  , SC AS (SELECT CNO, SNO, C.CNAME, RESULT
            FROM SCORE
            NATURAL JOIN COURSE C
            WHERE C.CNAME LIKE '%화학%'
            )
SELECT SNO                 --NATURAL조인에 사용된 열은 식별자를 가질 수 없다.
    , ST.SNAME
