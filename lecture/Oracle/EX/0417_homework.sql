...
PIVOT(AVG(AVR * 4.5/4.0) FOR MAJOR IN(
                                '화학' AS 화학,
                                '생물' AS 생물
                                )
                                );
...
SELECT *
    FROM STUDENT
    WHERE (MAJOR, SYEAR) IN (('화학', 1), ('화학', 2),('화학', 3)
                            ,('물리', 1),('물리', 2),('물리', 3));
...
WITH
    P AS (SELECT * FROM PROFESSOR WHERE ORDERS = '정교수'),
    C AS (SELECT * FROM COURSE WHERE CNAME LIKE '%일반%')
SELECT C.CNO
    , C.CNAME
    , P.PNO
    , P.PNAME
...
WITH
    ST AS(SELECT * FROM STUDENT WHERE AVR >= 3.3),
    SC AS(SELECT SNO, AVG(RESULT) AS RESULT FROM SCORE GROUP BY SNO) -- 밑에 SELECT절에서 단일 함수 쓰고 여기서도 쓰려면 별칭 설정해야 함.
SELECT  ST.SNO
      , ROUND(SC.RESULT ,2)
        FROM ST
            , SC
...
WITH
    P AS (SELECT * FROM PROFESSOR WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)/12) >= 25),
    A AS (SELECT * FROM COURSE NATURAL JOIN STUDENT NATURAL JOIN SCORE)
SELECT CNO
    , CNAME
    , SNO  
    , SNAME
