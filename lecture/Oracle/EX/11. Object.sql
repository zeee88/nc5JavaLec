...
--OWNER 컬럼이 없음.
--현재 접속한 사용자가 소유한 테이블의 정보만 담고 있다
SELECT * 
    FROM USER_TABLES;
    
--STUDY계정이 권한을 가지고 사용할 수 있는 모든 객체 정보
--STUDY계정에 DBA권한이 있어서 SYS가 소유하고 있는 테이블도 사용할 수 있다.
...
--세션을 KILL해서 접속 끊어버리는 쿼리(DB에 락걸렸을 때)
ALTER SYSTEM KILL SESSION ' SID번호 , SERIAL#번호' IMMEDIATE;
...
--1-2. 인덱스
--인덱스 생성
--STUDENT 테이블에 SNAME을 인덱스로 생성(비교유 인덱스)
...
--원천테이블의 데이터가 변경되면 뷰의 데이터도 자동으로 변경
UPDATE COURSE
    SET
        CNAME = '일반화학1'
        WHERE CNAME = '일반화학';
...
--조회쿼리가 1학년만 가져오는 쿼리라서 원천테이블 추가된 2, 3, 4학년의 데이터는 표출되지 않는다.
...
--인라인 뷰와 ROWNUM을 이용해서 최상위 N까지의 데이터 조회
--급여 최상위 3명 데이터 조회
SELECT ROWNUM
     , A.ENO
     , A.ENAME
...
--위치를 잘 지정해야 하고 최상위 SELECT구문에서의 ROWNUM은
-- <, <= 비교만 가능하다.
SELECT B.*
    FROM(
SELECT ROWNUM AS RN
...
--옵션을 하나도 추가하지 않은 시퀀스 생성
CREATE SEQUENCE DEPT_CO_CNO_SEQ1;

--옵션있는 시퀀스의 사용
--NEXTVAL : 시퀀스가 가리키는 값을 다음 값으로 옮긴다.
INSERT INTO EMP_COPY1
VALUES(EMP_CO_ENO_SEQ.NEXTVAL, 'H', '개발', 0, SYSDATE, 3000, 100, 0);

--CURRVAL : 시퀀스가 현재 가리키고 있는 값을 가져온다.
INSERT INTO EMP_COPY1
VALUES(EMP_CO_ENO_SEQ.CURRVAL, 'I', '개발', 0, SYSDATE, 3000, 100, 0);
...
VALUES(DEPT_CO_CNO_SEQ1.NEXTVAL, '개발', '서울', 0);
...
--시퀀스의 현재 값을 조회
SELECT EMP_CO_ENO_SEQ.CURRVAL FROM DUAL;

--시퀀스의 다음값을 조회
--NEXTVAL호출 시 무조건 시퀀스가 다음값으로 세팅됨
SELECT EMP_CO_ENO_SEQ.NEXTVAL FROM DUAL;


--시퀀스의 수정
ALTER SEQUENCE DEPT_CO_CNO_SEQ1
    INCREMENT BY 5
    MAXVALUE 100
    CYCLE;

--시퀀스의 삭제
DROP SEQUENCE DEPT_CO_CNO_SEQ1;
