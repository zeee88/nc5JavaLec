...
public class BoardDAO {
	...
	private final String UPDATE_BOARD = "UPDATE T_BOARD SET BOARD_CONTENT = ? WHERE BOARD_NO = ?";
	...
	private final String GET_BOARD2 = "SELECT * FROM T_BOARD";
	...
	public void updateBoard(BoardDTO boardDTO) {
		...
		try {
			...
			stmt.setString(1, boardDTO.getBoardContent());
			stmt.setInt(2, boardDTO.getBoardNo());
			stmt.executeUpdate();
	...
	public List<BoardDTO> getBoardList() {
		...
		try {
			...
			stmt = conn.prepareStatement(GET_BOARD2);
			
			rs = stmt.executeQuery();
			
			while(rs.next()) {	//다음행이 있으면 계속 꺼낸다
				BoardDTO boardDTO = new BoardDTO();
