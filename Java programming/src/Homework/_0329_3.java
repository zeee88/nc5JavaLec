package Homework;

import java.util.Scanner;

public class _0329_3 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

//		1. 사용자가 입력한 두 정수의 덧셈, 뺄셈, 곱셈, 나눗셈, 나머지 연산의 결과를 출력하세요.
//		  (메소드로 따로 만들지 말고 메인메소드에서 구현)

//		Scanner sc = new Scanner(System.in);
//		
//		System.out.println("2개의 정수를 입력하세요");
//		
//		 int i = sc. nextInt();
//		 int j = sc. nextInt();
//		 
//		System.out.println("덧셈 결과 = " + (i + j));
//		System.out.println("뺄셈 결과 = " + (i - j));
//		System.out.println("곱셈 결과 = " + (i * j));
//		System.out.println("나눗셈 결과 = " + (i / j));
//		System.out.println("나머지 결과 = " + (i % j));
		
		
		
		
//		2. int형 변수 20을 증감연산자를 사용해서 다음과 같은 순서로 출력되도록 구현하세요. 
//		21, 22, 22, 22, 20, 20, 19, 마지막 변수의 값(증감연산자 없이) : 20

//		int i = 20;
//		
//		System.out.println(++i);
//		System.out.println(++i);
//		System.out.println(i);
//		System.out.println(i--);
//		System.out.println(--i);
//		System.out.println(i--);
//		System.out.println(i++);
//		System.out.println(i);
		
		
		
//		3. 사용자가 입력한 정수가 2와 7의 공배수면 "2와 7의 공배수"라고 출력하고 아니면 "2와 7의 공배수가 아님"이라고 출력하세요.

//		Scanner sc = new Scanner(System.in);
//		System.out.println("정수를 입력하세요");
//		int input = sc.nextInt();
//		
//		if(input % 14 == 0) {
//			System.out.println("2와 7의 공배수");
//		}else
//			System.out.println("2와 7의 공배수가 아님.");
		
		
		
		
//		4. 사용자로부터 입력받은 문자열이 "Hello"인지 아닌지 판별하는 프로그램을 작성하세요.

//		Scanner sc = new Scanner(System.in);
//		System.out.println("문자열을 입력하세요");
//		String input = sc.next();
		
//		if(true) {
//			System.out.println("Hello가 맞ㅅ");   ???????????????????????????????????????
//			
//		}
		
//		switch   boolean
		
		
		
		
//		5. 삼항연산자를 이용하여 사용자가 입력한 정수가 홀수면 "홀수" 짝수면 "짝수"라고 출력하세요.

//		Scanner sc = new Scanner(System.in);
//		int input = sc.nextInt();
//		
//		System.out.println((input % 2 == 1)? "홀수" : "짝수");
		
		
		
		
//		6. 삼항연산자의 중첩을 이용하여 사용자가 입력한 정수가 100보다 크면 "100보다 큼" 작으면 "100보다 작음" 같으면 "100"을 출력하세요.

//		Scanner sc = new Scanner(System.in);
//		
//		System.out.println("정수를 입력하세요");
//		int input = sc.nextInt();
//		
//		System.out.println((input > 100)? "100보다 큼" : (input == 100)? "100" : "100보다 작음" );
		
		
		
	
		
//		7. 사용자가 입력한 두 실수의 덧셈, 뺄셈, 곱셈, 나눗셈, 나머지 연산의 결과를 출력하세요.

//		Scanner sc = new Scanner(System.in);
//		double i = sc.nextDouble();
//		double j = sc.nextDouble();
//		
//		System.out.println("2개의 실수를 입력하세요");
//		
//		System.out.println("덧셈 결과 = " + (i + j));
//		System.out.println("뺄셈 결과 = " + (i - j));
//		System.out.println("곱셈 결과 = " + (i * j));
//		System.out.println("나눗셈 결과 = " + (i / j));
//		System.out.println("나머지 결과 = " + (i % j));
		
		
		
		
//		8. 사용자가 입력한 하나의 문자열과 하나의 정수로 
//		"입력한 문자열 xxxxx이고, 입력한 숫자는 xxxxxx입니다." 라고 출력하세요.
		
//		Scanner sc = new Scanner(System.in);
//	
//		System.out.println("하나의 문자열과 하나의 정수를 입력하세요");
//		
//		String str = sc.next();
//		int i = sc.nextInt();
//		
//		System.out.println("입력한 문자열 " + str + "이고, 입력한 숫자는 " + i + "입니다.");
//		
		
		
		
		
		
		
		
		
	}

}
