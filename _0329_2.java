package Homework;

import java.util.Scanner;

public class _0329_2 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

//		1. 사용자가 두 개의 정수를 입력하면 문자열로 받아서 두 개의 문자열 정수를 정수형으로 변환해서 합을 출력하세요. 
		
//		Scanner sc = new Scanner(System.in);
//		
//		System.out.println("두 개의 정수를 입력하세요");
//		
//		int num1 = sc.nextInt();
//		int num2 = sc.nextInt();
//		
//		String str1 = String.valueOf(num1);
//		String str2 = String.valueOf(num2);		//String.valueOf();
//		
//		System.out.println((int)(num1 + num2));
		
		
		
		
//		2. 사용자가 두 개의 정수(a, b)를 입력해서 a / b의 연산결과를 실수로 출력하세요.(단, b는 0을 입력할 수 없습니다.)
		
//		Scanner sc = new Scanner(System.in);
//	
//		System.out.println("a / b를 계산하기 위해 두 개의 정수 a, b 입력하세요");
//		
//		int a = sc.nextInt();
//		int b = sc.nextInt();
//		
//		if(b==0) {
//			System.out.println("b에는 0을 입력할 수 없습니다.");
//		}else {
//			System.out.println((float)a/b);
//		}
		
		
		
	
//		3. 사용자가 실수 한 개를 입력하면 정수형으로 변환해서 출력하세요.
		
//		Scanner sc = new Scanner(System.in);
//		System.out.println("실수 하나를 입력하세요");
//		
//		float input = sc.nextInt();
//		
//		System.out.println((int)input);
		
		
		
		
//		4. 사용자가 한 개의 실수를 입력하면 그 실수의 제곱의 결과를 문자열로 변환하여 출력하세요.
		
//		Scanner sc = new Scanner(System.in);
//		System.out.println("실수 하나를 입력하세요");
//		
//		float input = sc.nextInt();
//		
//		System.out.println(String.valueOf(input * input));
		
		
		
//		5. int형 변수 iNum = 10과 long형 변수 lNum = 20의 값을 서로 교환하세요.
		
//		int iNum = 10;
//		long lNum = 20;
//		long tmp;
//		
//		tmp = lNum;
//		lNum = iNum;
//		iNum = (int)tmp;
//		System.out.println("iNum : " + iNum);
//		System.out.println("lNum : " + lNum);
		
		
//		6. 사용자가 입력한 두 개의 문자 각각의 유니코드를 출력하세요.

//		Scanner sc = new Scanner(System.in);
//		
//		System.out.println("두 개의 문자를 입력하세요");
//			
//		char [] arr = new char[5];
//		
//		for(int i = 0; i < arr.length; i++) {
//			arr[i] = sc.next().charAt(0);		//내가 입력한 문자열 중 0번째(charAt(0)) 있는 문자를 배열 arr[0]칸에 넣겠다.
//			System.out.println((int)arr[i]);
//		}
	
		
		
		
//		//char input  = sc.next().charAt(0);
//		//char input2  = sc.next().charAt(0);
//		//int unicode = (int)input;			//유니코드로 변환하려면 int형으로
//		//int unicode2 = (int)input2;
//		//
//		//	System.out.println(unicode);
//		//	System.out.println(unicode2);
		
		
	}



}
